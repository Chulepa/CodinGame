...
class Player {
    ...
    public class Ship{
        ...
        int targetHeightOverLandingPad = 400;
        int criticalVerticalSpeed = -33; //must be a negative value
        int toleranceOverMountain = 100;
        ...
        void update(){
            ...
            //VERTICAL VEL
            Vector2 tempV = land.calculateDangerPoint(pos, land.landingSite, toleranceOverMountain);
            if(tempV == null){
                
                tempV = land.landingSite.copy();
                tempV.y += targetHeightOverLandingPad;
            }else {
                mountFlag = true;
                System.err.println(tempV);
            }
            if(land.isOverLandingPad(pos, 1.0f)){
            ...
            //CALCULATE THRUST LEVEL, trancate to 0<=thrust<=4
            ...
            //FALLING TOO FAST?
            if(vel.y<criticalVerticalSpeed && !mountFlag) dAngle = 0;
    ...
    public class Land{
        ...
        public Vector2 calculateDangerPoint(Vector2 v, Vector2 t, float tolerance){
            Vector2 dp = null;
            for(Vector2 point: points){
                if(point == landingR || point == landingL) continue;
                if((point.x < t.x && point.x > v.x) || (point.x > t.x && point.x < v.x)){
                    if(point.y + tolerance > v.y){
                        if(dp == null) dp = new Vector2(point.x, point.y + tolerance);
                        else if(point.y + tolerance > dp.y) dp = new Vector2(point.x, point.y + tolerance);
                    }
                }
            }
        return dp;
